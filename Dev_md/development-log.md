...
### 개요
전공별 자료(`/fields/:fieldId`) 상세 페이지를 인터랙티브 2컬럼 레이아웃으로 전면 개편. 기존 단순 3섹션(개요/학술지/팁) 구조에서 각 분야별 4개 상세 섹션으로 확장.

### 변경 내용
- **FieldDetail.jsx**: 인터랙티브 2컬럼 레이아웃 적용 (좌측 네비 카드 + 우측 상세 패널)
  - 5개 전공 분야별 4개 상세 섹션 (논문 구조, 연구방법론, 학술지/인용, 실전 팁)
  - ko/en 이중 언어 상세 데이터 내장 (총 40개 콘텐츠 블록)
  - 분야별 테마 컬러 적용 (인문사회: 빨강, 자연과학: 파랑, 공학: 주황, 의학: 초록, 예술: 보라)
  - `← 전공별 자료` 돌아가기 버튼 추가
  - 기존 `structure-layout` CSS 클래스 재사용

### 파일 변경
| 파일 | 변경 |
|------|------|
| `src/pages/FieldDetail.jsx` | 전면 개편 (+999 / -77 lines) |
